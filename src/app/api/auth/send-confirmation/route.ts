...
export async function POST(request: Request) {
  ...
  if (hookSecret) {
    try {
      ...
      console.error('[send-confirmation] Signature invalide:', err);
  ...
  try {
    ...
    console.log('[send-confirmation] action:', payload?.email_data?.email_action_type, '| to:', payload?.user?.email);
    ...
    if (!email || !tokenHash) {
      console.error('[send-confirmation] Payload invalide:', body.slice(0, 300));
    ...
    let bodyText = "Merci de vous être inscrit sur <strong>FacturAvis</strong>. Cliquez sur le bouton ci-dessous pour confirmer votre adresse email et accéder à votre espace.";
    let btnLabel = 'Confirmer mon email';

    if (actionType === 'recovery') {
      subject = 'Réinitialisation de votre mot de passe — FacturAvis';
      heading = 'Réinitialiser votre mot de passe';
      bodyText = 'Vous avez demandé une réinitialisation de mot de passe. Cliquez sur le bouton ci-dessous. Ce lien expire dans 1 heure.';
      ...
      html: `
        ...
        <body style="margin:0;padding:0;background-color:#f7f4f1;font-family:'Helvetica Neue',Arial,sans-serif;color:#3e2f25;">
          <div style="max-width:560px;margin:40px auto;background:#fff;border-radius:16px;padding:40px;border-top:5px solid #a9825a;box-shadow:0 4px 20px rgba(0,0,0,0.06);">
            ...
            <p style="font-size:15px;line-height:1.6;color:#5a4a40;margin:0 0 28px;">${bodyText}</p>
            <div style="text-align:center;margin:32px 0;">
              <a href="${confirmUrl}" style="display:inline-block;background:#a9825a;color:#ffffff;padding:14px 32px;border-radius:10px;text-decoration:none;font-weight:700;font-size:15px;">
                ${btnLabel}
              </a>
            </div>
    ...
    if (resendError) {
      console.error('[send-confirmation] Resend error:', resendError);
      return NextResponse.json({ error: 'Erreur envoi email' }, { status: 500 });
    }

    console.log('[send-confirmation] Email envoyé à', email);
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('[send-confirmation] Erreur:', error);
