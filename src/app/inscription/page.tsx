...
import { Loader2, ArrowRight, ArrowLeft, Lock, Mail, Eye, EyeOff, CheckCircle } from 'lucide-react';
...
export default function InscriptionPage() {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      // 2. Create minimal therapeute profile via server API (bypasses RLS).
      // Le reste du profil (nom, profession, SIRET, cabinet...) est complété
      // après connexion, dans l'onboarding du dashboard.
      ...
      // 3. Fire Google Ads conversion (inscription gratuite)
          ...
          <div className="bg-[#fdf2e9] rounded-2xl p-6 border border-[#f0e6de] mb-8 text-left">
            ...
            <ul className="space-y-2 text-sm text-[#7a6a5f]">
              ...
              <li className="flex items-start gap-2">
                ...
                Complétez votre profil (30 secondes) et activez la réception de factures fournisseurs (gratuit)
              </li>
            </ul>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#fcfaf8] flex flex-col justify-center items-center p-4 font-sans text-[#3e2f25]">
      <div className="mb-8 text-center">
        ...
        <p className="text-[#7a6a5f] font-medium mt-1">30 secondes. Le reste se complète après connexion.</p>
      </div>
      ...
      <div className="bg-white w-full max-w-lg rounded-[2rem] shadow-2xl shadow-[#a9825a]/10 p-8 md:p-10 border border-[#f0e6de]">
        <div className="space-y-5">
          ...
          <button onClick={handleSignup} disabled={loading}
            className="w-full bg-[#3e2f25] text-white py-4 rounded-2xl font-black text-lg hover:bg-black transition-all flex items-center justify-center gap-2 disabled:bg-gray-300">
            {loading ? <Loader2 size={20} className="animate-spin" /> : <>Créer mon espace <ArrowRight size={20} /></>}
          </button>
        </div>
