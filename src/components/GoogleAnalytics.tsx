...
export default function MerciPage() {
  ...
  useEffect(() => {
    // On s'assure qu'on est bien côté client
    if (typeof window !== 'undefined') {
      console.log('Déclenchement de la conversion Google Ads...');

      const w = window as any;

      // 1. On initialise le dataLayer s'il n'existe pas encore
      w.dataLayer = w.dataLayer || [];

      // 2. On crée une fonction gtag locale qui pousse dans le dataLayer
      // Cela court-circuite le problème si le script externe Google n'a pas fini de charger
      function gtag(...args: any[]) {
        w.dataLayer.push(args);
      }

      // 3. On envoie l'événement de conversion !
      ...
      console.log('Conversion envoyée avec succès dans le dataLayer !');
    }
  }, []);
